...
type Props = {
  /** 목록에서 선택한 식당만 전달. null이면 지도에 핀 없음 */
...
export function KakaoMap({ selected, appKey }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  const mapRef = useRef<KakaoMapHandle | null>(null);
  const markerRef = useRef<KakaoMarkerHandle | null>(null);
  ...
  useEffect(() => {
    ...
    const ensureKakao = (): Promise<void> =>
      new Promise((resolve, reject) => {
        ...
        if (existing) {
          const done = () => window.kakao.maps.load(() => resolve());
          if (window.kakao?.maps) {
            done();
            return;
          }
  ...
  useEffect(() => {
    ...
    const cached = coordsCacheRef.current.get(r.id);
    if (cached) {
    ...
    if (typeof r.lat === "number" && typeof r.lng === "number") {
      ...
      coordsCacheRef.current.set(r.id, coords);
      showPinAt(coords.lat, coords.lng);
      return () => {
        cancelled = true;
        removePin();
      };
